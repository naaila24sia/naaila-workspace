      ...
      <div className="bg-white rounded-[2.5rem] border border-border shadow-soft overflow-hidden">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-bg-main/50 border-b border-border">
                <th className="px-8 py-5 text-[11px] font-black uppercase tracking-widest text-text-soft">Pet Patient</th>
                <th className="px-6 py-5 text-[11px] font-black uppercase tracking-widest text-text-soft">Owner</th>
                <th className="px-6 py-5 text-[11px] font-black uppercase tracking-widest text-text-soft">Service</th>
                <th className="px-6 py-5 text-[11px] font-black uppercase tracking-widest text-text-soft">Date & Time</th>
                <th className="px-6 py-5 text-[11px] font-black uppercase tracking-widest text-text-soft">Status</th>
            ...
            <tbody className="divide-y divide-border text-sm">
              {filteredAppointments.length > 0 ? (
                filteredAppointments.map((item) => (
                  <tr key={item.id} className="hover:bg-bg-main/30 transition-colors group">
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="font-poppins font-black text-text-main text-base uppercase leading-tight">{item.pet}</p>
                          <p className="text-[10px] font-bold text-primary uppercase tracking-tighter mt-0.5">{item.type}</p>
                        </div>
                      </div>
                    </td>

                    <td className="px-6 py-5">
                      <p className="font-poppins font-bold text-text-main text-sm leading-tight">{item.owner}</p>
                      <p className="text-[10px] text-text-soft/60 font-bold uppercase tracking-wider mt-0.5">Pet Parent</p>
                    </td>
                    ...
                    <td className="px-6 py-5">
                      <p className="font-black text-text-main text-sm leading-tight">{item.date}</p>
                      <p className="text-[10px] text-primary font-black uppercase italic tracking-tighter mt-0.5">Confirmed</p>
                    </td>
                    ...
                    <td className="px-6 py-5">
                      <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl border shadow-sm ${
                        ...
                        <div className={`w-1.5 h-1.5 rounded-full ${item.status?.toLowerCase() === "done" ? "bg-success-text" : "bg-accent-text"}`}></div>
                ...
                <tr>
                  <td colSpan="6" className="px-8 py-20 text-center">
                    <p className="text-text-soft font-black uppercase tracking-widest text-xs">No {statusFilter} appointments found.</p>
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <div className="p-6 bg-bg-main/20 border-t border-border flex justify-between items-center">
          <p className="text-xs text-text-soft font-bold uppercase tracking-widest">
            Showing <span className="text-primary">{filteredAppointments.length}</span> results
          </p>
          <div className="flex gap-2">
            <button className="px-5 py-2 bg-white border border-border rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-bg-main transition-all shadow-sm">Prev</button>
            <button className="px-5 py-2 bg-white border border-border rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-bg-main transition-all shadow-sm">Next</button>
          </div>
        </div>
      </div>
    </div>
